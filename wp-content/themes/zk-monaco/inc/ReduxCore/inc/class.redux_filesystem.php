        ...
        class Redux_Filesystem {
            ...
            public function __construct() {
                $this->parent->admin_notices[] = array(
                    'type'    => 'error',
                    'msg'     => '<strong>' . esc_html__( 'File Permission Issues', "zk-monaco" ) . '</strong><br/>' . sprintf( esc_html__( 'We were unable to modify required files. Please check your permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%s" target="_blank">outlined here</a>.', "zk-monaco" ), 'https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants' ),
                    'id'      => 'redux-wp-login',
                    'dismiss' => false,
                );
            }
            ...
            public function ftp_form() {
                if ( isset( $this->parent->ftp_form ) && ! empty( $this->parent->ftp_form ) ) {
                    ...
                    echo '<strong>' . esc_html__( 'File Permission Issues', "zk-monaco" ) . '</strong><br/>' . sprintf( esc_html__( 'We were unable to modify required files. Please ensure that <code>%1s</code> has the proper read-write permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%2s" target="_blank">outlined here</a>.', "zk-monaco" ), Redux_Helpers::cleanFilePath( trailingslashit( WP_CONTENT_DIR ) ) . '/uploads/', 'https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants' );
            ...
            public function execute( $action, $file = '', $params = '' ) {
                ...
                $this->filesystem_init( $url, 'direct', get_template_directory() . '/inc/ReduxCore/inc' );
