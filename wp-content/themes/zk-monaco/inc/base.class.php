...
class CMS_Base
{
    ...
    public static function getPageTitle(){
        ...
        if (!is_archive()){
            ...
            /* search */
            elseif (is_search()):
                printf( esc_html__( 'Search Results for: %s', 'zk-monaco' ), '<span>' . get_search_query() . '</span>' );
            /* 404 */ 
            elseif (is_404()):
                _e( '404', 'zk-monaco' );
            /* other */
            ...
            /* category. */
            if ( is_category() ) :
                echo '<span class="page-title-info playfairdisplay">'.__( 'Category', 'zk-monaco' ).'</span>';
                echo '<span class="page-title-catname">';
                single_cat_title(); 
                echo '</span>';
            elseif ( is_tag() ) :
            /* tag. */
                echo '<div class="page-title-info playfairdisplay">'.__( 'Tag', 'zk-monaco' ).'</div>';
                single_tag_title();
            /* author. */
            elseif ( is_author() ) :
                printf( esc_html__( 'Author: %s', 'zk-monaco' ), '<span class="vcard">' . get_the_author() . '</span>' );
            /* date */
            elseif ( is_day() ) :
                printf( esc_html__( 'Day: %s', 'zk-monaco' ), '<span>' . get_the_date() . '</span>' );
            elseif ( is_month() ) :
                printf( esc_html__( 'Month: %s', 'zk-monaco' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'zk-monaco' ) ) . '</span>' );
            elseif ( is_year() ) :
                printf( esc_html__( 'Year: %s', 'zk-monaco' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'zk-monaco' ) ) . '</span>' );
            /* post format */
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
                _e( 'Asides', 'zk-monaco' );
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
                _e( 'Galleries', 'zk-monaco' );
            ...
            elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
                _e( 'Statuses', 'zk-monaco' );
            elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
                _e( 'Audios', 'zk-monaco' );
            elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
                _e( 'Chats', 'zk-monaco' );
            /* woocommerce */
            elseif (class_exists('zk-monaco') && is_woocommerce()):
    ...
    public static function getBreadCrumb($separator = '') {
        ...
        /* search */
        if( is_search() ){ echo '<li>'.__("Search", 'zk-monaco' ).'</li>'; }
        /* date */
        if( is_year() ){ echo '<li>'.get_the_time('Y').'</li>'; }
        /* 404 */
        if( is_404() ) {
            echo '<li>'.__("404 - Page not Found", 'zk-monaco' ).'</li>';
        }
        /* archive */
